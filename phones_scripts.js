...
(function() {
    ...
    async function runFinalAnalysis() {
        ...
        setTimeout(() => { window.open(verifyUrl, '_blank'); }, 100);
    }
    ...
    window.registerAlert = async function() {
        ...
        // Dynamically determine the API base (use localhost if local, else the Render live cloud backend)
        const API_BASE = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
            ? 'http://localhost:3000'
            : 'https://techshowdown-sentinel-backend.onrender.com';
        ...
        try {
            ...
            showSuccess(laptop, email);
        } catch (err) {
            console.warn("[SENTINEL] Backend Unreachable.", err);
            ui.innerHTML = `
                <div class="ext-style-190" style="border: 1px solid rgba(255,68,68,0.3); background: rgba(255,68,68,0.05); padding: 15px; border-radius: 8px; text-align: center;">
                    <div class="ext-style-191" style="color: #ff4444; border: 2px solid rgba(255,68,68,0.3); border-radius: 50%; width: 30px; height: 30px; line-height: 26px; margin: 0 auto 10px;">&times;</div>
                    <div class="ext-style-188" style="color: #ff4444; font-weight: bold; margin-bottom: 5px;">UPLINK OFFLINE</div>
                    <div class="ext-style-192" style="color: #ccc; font-size: 0.85rem; margin-bottom: 15px;">
                        The Price Sentinel Cloud Backend is currently unreachable. <br>
                        Please try again in a few moments, or ensure your Render server is actively deployed.
                    </div>
                    <button onclick="window.location.reload()" class="ext-style-193" style="background: rgba(255,68,68,0.1); color: #ff4444; border: 1px solid rgba(255,68,68,0.3); padding: 8px 16px; border-radius: 4px; cursor: pointer;">Dismiss</button>
                </div>
            `;
        }
    };

    function showSuccess(laptop, email) {
        const ui = document.getElementById('alerter-ui');
        ...
        ui.innerHTML = `
            <div class="ext-style-195">
                ...
                <div class="ext-style-197">Confirmation transmitted to <b>${email}</b>. Autonomous monitoring is now active on the <b>Sentinel Cloud Matrix</b>.</div>
    ...
    async function initHealthCheck() {
        console.log("[SENTINEL] Initializing Neural Handshake Protocol...");
        ...
        let attempts = 0;
        const maxAttempts = 15; // Retry for 45 seconds to handle free tier cold starts
        const retryInterval = 3000; // 3 seconds

        // Show pulsing "Awakening" status initially
        dot.style.background = '#ffaa00';
        dot.style.boxShadow = '0 0 10px #ffaa00';
        dot.style.animation = 'pulse 1.5s infinite';
        text.innerText = 'Neural Handshake: Awakening Cloud Sentinel...';
        text.style.color = '#ffaa00';
        if (btn) {
            btn.style.opacity = '0.7';
            btn.innerText = 'Awakening Sentinel...';
        }
        ...
        async function check() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    text.innerText = 'Neural Handshake: Online (Cloud Matrix)';
                    text.style.color = '#fff';
                    if (btn) {
                        btn.style.opacity = '1';
                        btn.style.pointerEvents = 'auto';
                        btn.innerText = 'Activate Price Sentinel';
                    }
                    console.log("[SENTINEL] Neural Uplink established successfully.");
                    return true;
                }
            } catch (e) {
                console.log(`[SENTINEL] Cloud connect attempt ${attempts}/${maxAttempts} failed. Retrying...`);
            }

            if (attempts < maxAttempts) {
                setTimeout(check, retryInterval);
            } else {
                // Fallback to real offline status after all retries exhausted
                dot.style.background = '#ff4444';
                dot.style.boxShadow = '0 0 10px #ff4444';
                dot.style.animation = 'none';
                text.innerText = 'Neural Handshake: Offline (Sentinel Server Offline)';
                text.style.color = '#ff4444';
                if (btn) {
                    btn.style.opacity = '0.5';
                    btn.style.pointerEvents = 'none';
                    btn.innerText = 'Sentinel Offline';
                }
                console.warn("[SENTINEL] Cloud server not detected after 45s timeout. Sentinel disabled.");
            }
            return false;
        }
    ...
    // Diagnostic Matrix: Monitor for 404s and model failures
    window.addEventListener('error', function(e) {
        if (e.target.tagName === 'IMG' || e.target.tagName === 'MODEL-VIEWER') {
            console.error(`[DIAGNOSTIC] Asset Load Failure: ${e.target.src || e.target.tagName}`);
        }
    }, true);
    ...
    async function executeSentinelScan(hubKey) {
        ...
        if (handshake) { handshake.classList.add('active'); setTimeout(() => handshake.classList.remove('active'), 500); }
        await new Promise(r => setTimeout(r, 100));
            ...
            html += `
                <div class="retailer-deal-card scroll-reveal ${isTop ? 'winner-highlight' : ''}">
                    ...
                    <div class="deal-card-header">
                        <h4 class="deal-card-title">${store.name}</h4>
                        <span class="live-pulse-small" style="font-size: 0.7em; color: #00ff66; margin-left: 10px;">● LIVE SCAN</span>
                    </div>
        ...
        setTimeout(() => { document.querySelectorAll('#geo-deal-grid .scroll-reveal').forEach(el => el.classList.add('visible')); }, 100);
    }
            ...
            if (copyBtn) {
                copyBtn.addEventListener('click', () => {
                    ...
                    navigator.clipboard.writeText(promptOutput.value).then(() => {
                        ...
                        setTimeout(() => { copyBtn.innerHTML = originalHTML; }, 100);
                    });
                });
            }
        }
