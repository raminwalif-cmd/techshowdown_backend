...
function buildPriceChangeEmail(deviceName, scanResults) {
    ...
    for (const result of scanResults) {
        ...
        let changeLabel = '';
        let changeLabelColor = '#94a3b8';
        let prevPriceHtml = '';

        if (result.priceDropped) {
            changeLabel = 'PRICE DROPPED';
            changeLabelColor = '#00ff66';
        } else if (result.priceIncreased) {
            changeLabel = 'PRICE INCREASED';
            changeLabelColor = '#ff4444';
        } else if (result.priceChanged) {
            changeLabel = 'PRICE CHANGED';
            changeLabelColor = '#ffaa00';
        }

        if (result.previousPrice !== null && result.priceChanged) {
            prevPriceHtml = `<p style="margin:0 0 6px;font-size:13px;color:#64748b;">Was: <span style="text-decoration:line-through;">$${result.previousPrice.toFixed(2)}</span></p>`;
        }
        ...
        <div style="background:#0f172a;border:1px solid #1e293b;border-radius:12px;padding:24px;margin-bottom:16px;">
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td>
                <p style="margin:0 0 4px;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#475569;font-weight:600;">Retailer</p>
                <p style="margin:0;font-size:18px;font-weight:700;color:#ffffff;">${result.retailer}</p>
              </td>
              <td align="right" valign="top">
                <span style="background:${changeLabelColor}22;color:${changeLabelColor};border:1px solid ${changeLabelColor}44;padding:4px 10px;border-radius:6px;font-size:11px;font-weight:700;letter-spacing:1px;">${changeLabel}</span>
              </td>
            </tr>
          </table>
          <hr style="border:none;border-top:1px solid #1e293b;margin:16px 0;">
          ${prevPriceHtml}
          <p style="margin:0 0 20px;font-size:28px;font-weight:800;color:#00d2ff;">$${result.price.toFixed(2)}<span style="font-size:13px;font-weight:400;color:#475569;"> AUD</span></p>
          <a href="${result.url}" style="display:inline-block;background:#00d2ff;color:#0b0f1a;text-decoration:none;font-weight:700;font-size:13px;letter-spacing:1px;padding:12px 24px;border-radius:8px;">VIEW CURRENT PRICE</a>
        </div>`;
    }
...
function buildDealEmail(deviceName, scanResults) {
    ...
    for (const result of scanResults) {
        ...
        const dealDetails = result.deals.map(deal => `
            <div style="background:#1a2235;border-radius:8px;padding:12px 16px;margin-bottom:10px;">
              <p style="margin:0 0 6px;"><span style="background:#ffaa0022;color:#ffaa00;border:1px solid #ffaa0044;padding:3px 8px;border-radius:4px;font-size:11px;font-weight:700;letter-spacing:1px;">${deal.keyword.toUpperCase()}</span></p>
              <p style="margin:0;font-size:13px;color:#94a3b8;line-height:1.5;">${deal.context}</p>
            </div>
        `).join('');
        ...
        <div style="background:#0f172a;border:1px solid #1e293b;border-radius:12px;padding:24px;margin-bottom:16px;">
          <p style="margin:0 0 4px;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#475569;font-weight:600;">Retailer</p>
          <p style="margin:0 0 16px;font-size:18px;font-weight:700;color:#ffffff;">${result.retailer}</p>
          <hr style="border:none;border-top:1px solid #1e293b;margin:0 0 16px;">
          <p style="margin:0 0 10px;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#475569;font-weight:600;">Deal Details</p>
          ${dealDetails}
          ${currentPrice}
